...
public class SpriteTestScreen extends Screen {
	...
	private Map map1_layer1 = new Map(r);
    private Map map1_layer2 = new Map(r);
    private Map fogOfWar = new Map(r);
    ...
    public SpriteTestScreen(final Registry r) {
		super(r);
	}
    ...
    @Override
	public void show() {
		final TextureAtlas spritesAtlas = r.assetManager.<TextureAtlas>get(Asset.spritesAtlas);
		debugFont = r.assetManager.get(Asset.mono16Font);
	    shoutFont = r.assetManager.get(Asset.sans24boldFont);
        ...
        hero = new HeroUnit(r, spritesAtlas);
	...
	private void graphics(final float delta, final float alpha) {
		...
		*/
		if (hero.isShouting()) {
			...
			drawCentred(shoutFont, batch, "Orcs! Go " + hero.computeShoutCommand() + "!", hero.getDrawX() - viewport.viewportX + 2, hero.getDrawY() - viewport.viewportY - 17);
			shoutFont.setColor(1, 1, 1, 1);
			drawCentred(shoutFont, batch, "Orcs! Go " + hero.computeShoutCommand() + "!", hero.getDrawX() - viewport.viewportX, hero.getDrawY() - viewport.viewportY - 15);
		}
		batch.end();
	}
    ...
    public void scheduleEnemySpawn(int numberToSpawn, int tileX, int tileY) {
	    final TextureAtlas spritesAtlas = r.assetManager.get(Asset.spritesAtlas);
        ...
        for (int i=0;i<numberToSpawn;i++) {
	        ...
	        if (enemyType < 1) {
		        toSpawn.add(new MosquitoUnit(r, spritesAtlas, spawnX, spawnY));
	        } else if (enemyType < 2) {
		        toSpawn.add(new WaspUnit(r, spritesAtlas, spawnX, spawnY));
	        } else if (enemyType < 3) {
		        toSpawn.add(new LobsterUnit(r, spritesAtlas, spawnX, spawnY));
	        } else {
		        toSpawn.add(new CricketUnit(r, spritesAtlas, spawnX, spawnY));
	        }
        }
    ...
    public void scheduleFriendlySpawn(int numberToSpawn) {
	    ...
	    for (int i=0;i<numberToSpawn;i++) {
		    ...
		    if (friendlyType < 1) {
			    toSpawn.add(new LeatherUnit(r, spritesAtlas, spawnX, 4));
		    } else if (friendlyType < 2) {
			    toSpawn.add(new MailUnit(r, spritesAtlas, spawnX, 4));
		    } else {
			    toSpawn.add(new ClothUnit(r, spritesAtlas, spawnX, 4));
		    }
        }
	...
	private void control(final float delta) {
		...
		final MusicMan musicMan = r.game.getMusicMan();
		...
		if (!hero.isAlive() /* || gates are down */) {
			r.game.setScreen(new LoseEndSplashScreen(r));
		}
		if (demonGatesLeft == 0) {
			r.game.setScreen(new WinEndSplashScreen(r));
		}
	}
