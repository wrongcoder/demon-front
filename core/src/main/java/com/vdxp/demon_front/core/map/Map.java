...
public class Map {
    ...
    public Map(final Registry r) {
		this.r = r;
    }

    public Map(final Registry r, final String asciiSourcePath) {
	    this(r);
        ...
        for (int i=0;i<asciiSource.length();i++) {
            ...
            switch (currChar) {
                ...
                case 'M':
                    ...
                    break;
                case 'd':
                    units.add(new DemonGate(r, spritesAtlas, currX, currY));
                    break;
                case 'w':
                    units.add(new WallSection(r, spritesAtlas, currX, currY));
                    break;
                case 'F':
                    if ((currX*currY) % 2 == 0) {
                        nonCollidableMapTiles.add(new FogOfWarTile(r, spritesAtlas, currX, currY));
                    } else {
                        nonCollidableMapTiles.add(new FogOfWarWFaceTile(r, spritesAtlas, currX, currY));
                    }
                    break;
            }
