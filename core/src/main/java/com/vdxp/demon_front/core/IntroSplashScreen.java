...
public abstract class IntroSplashScreen extends Screen {
	...
	public IntroSplashScreen(final Registry r, final Screen nextScreen) {
		super(r);
	...
	public void render(final float delta) {
		...
		batch.begin();
		drawCentred(smallFont, batch, "Move Hero", arrowsBaseX + 16, arrowsBaseY + 50);
		drawCentred(smallFont, batch, "Command Orcs", wasdBaseX + 16, wasdBaseY + 50);
		keyW.draw(batch);
		keyA.draw(batch);
		keyS.draw(batch);
		keyD.draw(batch);
		...
		keyRight.draw(batch);
		keyDown.draw(batch);
		drawCentred(smallFont, batch, "Orcs will shuffle aimlessly unless commanded", Registry.WIDTH / 2, 230);
		drawCentred(smallFont, batch, "Commands affect all orcs that the hero can clearly see", Registry.WIDTH / 2, 210);
		drawCentred(smallFont, batch, "Destroy all the demon gates, but stay alive and don't let the demons enter your city", Registry.WIDTH / 2, 190);
		batch.end();
	}
