...
public abstract class EnemyUnit extends Unit {
    ...
    public EnemyUnit(final Registry r, final TextureAtlas spritesAtlas, final int spriteId, final float maxHp, final float width, final float height, final int xTile, final int yTile) {
		super(r, maxHp);
	...
	public void drawSprite(final SpriteBatch batch, final Viewport viewport, final float delta, final float alpha) {
		...
		if (swordSlashTimer > delta) {
            ...
            r.game.getSoundMan().playHumanAttack();
		}
	}
